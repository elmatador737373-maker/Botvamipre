...
def init_db():
    ...
    try:
        ...
        # --- 4. FAZIONI (DEPOSITI SOLDI E OGGETTI) ---
        cur.execute("""
        ...
        # --- 9. FEDINA PENALE (ARRESTI) ---
        cur.execute("""
            CREATE TABLE IF NOT EXISTS arresti (
                id_arresto SERIAL PRIMARY KEY,
                user_id TEXT,
                agente_id TEXT,
                motivo TEXT,
                tempo INTEGER,
        ...
        # --- 11. RACCOLTA MATERIALI (SESSIONI) ---
        cur.execute("""
            CREATE TABLE IF NOT EXISTS sessioni_raccolta (
                user_id TEXT PRIMARY KEY,
                cosa_raccoglie TEXT,
                inizio_timestamp TIMESTAMP
            )
        """)
        ...
        conn.commit()
        print("✅ Database inizializzato con tutte le tabelle (12/12)!")
...
async def clear(interaction: discord.Interaction, quantita: int):
    ...
    ID_RUOLO_AUTORIZZATO = 1477011871025594510
...
async def arresto(interaction: discord.Interaction, utente: discord.Member, tempo_minuti: int, motivo: str):
    ...
    if not any(role.id == 1477011871025594510 for role in interaction.user.roles):
...
ID_RUOLO_CONCESSIONARIO = 1477011304341442591
